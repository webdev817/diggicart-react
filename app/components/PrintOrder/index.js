...
class PrintOrder extends React.Component {
  ...
  resizeSignature = async () => {
    ...
    resizedContext.drawImage(canvas, 0, 0, signatureWidth, signatureHeight); // signatureWidth X signatureHeight
  ...
  attachSignature = async () => {
    ...
    const mainImage = await this.resizeDrawingImg();
    await watermark([mainImage, signatureImage], options)
  ...
  resizeDrawingImg = async (isOriginalImg = false) => {
    if (isOriginalImg) {
    ...
    const canvas = document.querySelector("#reference > canvas"); // Original drawing canvas
    resizedContext.drawImage(canvas, 0, 0, width, height); // width X height

    return await resizedCanvas.toDataURL("image/png", 1);
  }
